...
def estimate(
    ...
    try:
        ...
        return {"op": op, "to": to_ck, "amount": str(amt), "gas": int(gas), "gasPriceWei": int(tx["gasPrice"]), "chain_id": CHAIN_ID}
    except Exception as e:
...
def _send_tx(fn):
    ...
    return {"txHash": txh.hex(), "gas": int(gas), "gasPriceWei": int(tx["gasPrice"]), "chain_id": CHAIN_ID}
