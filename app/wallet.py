...
async def _rpc_call(method: str, params: list) -> Optional[str]:
    """
    קריאת JSON-RPC גנרית ל-BSC.
    מחזירה את השדה 'result' או None במקרה של שגיאה.
    """
    payload = {
        "jsonrpc": "2.0",
        "id": 1,
...
def _decode_hex_to_decimal(value_hex: Optional[str], decimals: int) -> Decimal:
    """
    המרה של מספר hex ל-Decimal לפי כמות ה-decimals של הטוקן.
    """
    if not value_hex:
        return Decimal("0")
...
async def _fetch_slh_balance(address: str) -> Decimal:
    """
    החזרת יתרת SLH on-chain לפי החוזה.
    """
    ...
    try:
        decimals = int(getattr(settings, "SLH_TOKEN_DECIMALS", 18))
    except Exception:
        decimals = 18
...
async def _get_internal_slh_balance(db: Session, telegram_id: str) -> Decimal:
    """
    יתרה פנימית (INTERNAL) מחושבת מתוך טבלת transactions.
    incoming - outgoing עבור אותו משתמש.
    """
    incoming = (
...
async def get_balances_live(wallet: models.Wallet, db: Session) -> schemas.BalancesOut:
    ...
    # יש BNB – נמשוך גם BNB וגם SLH מהחוזה
...
async def set_wallet(
    ...
    """
    יצירה / עדכון של ארנק משתמש:
    - bnb_address
    - slh_address (ברירת מחדל = bnb_address)
    - ton_address
    - username / first_name
    """
    wallet = (
    ...
    if wallet is None:
        ...
        wallet.slh_address = (
            payload.slh_address
            or wallet.slh_address
            or wallet.bnb_address
        )
